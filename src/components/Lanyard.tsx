...
'use client';
// react-compiler memoization breaks r3f/rapier's mutable per-frame refs
'use no memo';
...
function Band({
  ...
  // Placeholder must be a real image file: 1x1 data-URI textures can throw
  // "texSubImage2D: bad image data" on some GPUs and kill the WebGL context.
  ...
  useFrame((state, delta) => {
    ...
    if (fixed.current) {
      [j1, j2].forEach(ref => {
        ...
        // Clamp to 1: a large delta (slow first frame) otherwise extrapolates past
        // the target, positions explode to NaN and the WebGL context is lost.
